...
//    val csr_status = UInt(INPUT, width=nret*xlen)
  ...
  def connect(content: Vec[RVFIMonitor.RVFI_Base]): Unit = {
    ...
    io.rvfi_csr_misa_rmask := SInt(-1, width=xlen).asUInt
...
//  inst_commit.mcycle_wdata := csr.io.rw.wdata
  inst_commit.mcycle_wdata := csr.readModifyWriteCSR(csr.io.rw.cmd, csr.io.rw.rdata, csr.io.rw.wdata)
  ...
  inst_commit.retire_wdata := csr.readModifyWriteCSR(csr.io.rw.cmd, csr.io.rw.rdata, csr.io.rw.wdata)
  ...
  inst_commit.misa_wdata := csr.readModifyWriteCSR(csr.io.rw.cmd, csr.io.rw.rdata, csr.io.rw.wdata)
  ...
  when(mem_wvalid) {
    inst_commit.mem_wmask := SInt(-1, width=p(XLen)/8).asUInt
